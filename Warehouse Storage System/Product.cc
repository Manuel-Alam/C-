...
//overloaded constructor used to initialize member variables of a product object.
...
//destructor used to delete all the whLocations.
Product::~Product()
{
	delete whLocations;
}

//getter used to return name of product.
const string& Product::getName() const
{
	return name;
}

//getter used to return storelocation.
StoreLocation* Product::getStoreLocation() const
{
	return storeLocation;
}

//setter used to setter the storelocation.
void Product::setStoreLocation(StoreLocation* sloc)
{
	storeLocation = sloc;
}

//adds a whlocation to the queue.
void Product::addWHLocation(WHLocation* wloc)
{
	whLocations->addLast(wloc);
}
